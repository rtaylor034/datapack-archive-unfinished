...
2. After completing `Pack`, `Developer` runs [getinstallcommands](Packs/SlimeCore%20v2/data/slimec/functions/api/admin/getinstallcommands.mcfunction) providing specified information about `Pack`, including a download/repo link.
...
I believe the nail in the coffin was when I got stumped on the issue of pack execution order. There was no way to control execution order without asking developers to jump through more hoops than I would like. Without controlling execution order there would be fundamental pack installation issues.
