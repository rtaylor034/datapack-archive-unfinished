...
execute store result storage cmd:process/array/cycleto tag.id int 1 run scoreboard players get unequip.id var
...
execute if score unequip.success var matches 1 run data modify storage cmd:var unequip.slots[0] set value {slot:0,time:0}
execute if score unequip.success var matches 1 run data modify storage cmd:var unequip.slots[0].slot set from storage cmd:var unequip.out.slot
